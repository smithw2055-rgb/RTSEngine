...
public:
    ...
    bool setBlockedBatch(const std::vector<GridPoint>& points, bool value) {
        ...
        if (changed) commitTopologyChange();
        return true;
    }
...
private:
    void commitTopologyChange() noexcept {
        ++revision_;
...
public:
    ...
    static PathResult find(
        ...
        std::uint32_t nodeBudget = 4096) {
        if (!grid.contains(goal) || grid.blocked(goal)) return {};
        return findInternal(
            grid,
            start,
            scratch,
...
private:
    ...
    static PathResult findInternal(
        ...
        Heuristic&& heuristic) {
        PathResult result;
        if (!grid.contains(start) || grid.blocked(start)) return result;
        if (goal(start)) {
            result.found = true;
            return result;
        }
        ...
        if (reachedIndex < 0) return result;
