...
class RtsScriptLockstepSession final {
public:
    RtsScriptLockstepSession(
    ...
    const realscript::runtime::RuntimeError& lastScriptError() const noexcept {
        return lastScriptError_;
    }
    ...
    RtsLockstepStartResult start() {
        ...
        if (config_.sessionId == 0 || config_.checkpointCapacity == 0 ||
            config_.maximumCommandsPerFrame == 0 || !scripts_.valid()) {
    ...
    bool buildLocalFrame(
        ...
        output.tick = coordinator_.scheduledTick(coordinator_.simulatedThrough());
    ...
    RtsLockstepAdvanceResult advanceOne() {
        ...
        if (session_.stepDetailed(tick) != RtsStepResult::Advanced ||
            !runScripts(tick) || !coordinator_.markSimulated(tick) ||
    ...
    bool makeHashReport(
        ...
        output = {config_.sessionId, peerId, tick, local->worldHash};
        return true;
    }
    ...
    bool makeReconnectSnapshot(RtsReconnectSnapshot& output) const {
        if (!started_ || coordinator_.rollbackRequired() ||
            coordinator_.confirmedThrough() < coordinator_.simulatedThrough()) {
            return false;
        }
        output = {};
        ...
        if (!RtsScriptSessionArchive::authoritativeHash(
                session_, scripts_, output.authoritativeHash, error)) {
            return false;
        }
        ...
        output.sessionArchive = RtsScriptSessionArchive::encode(
            session_, scripts_, error);
    ...
    bool restoreReconnectSnapshot(RtsReconnectSnapshot snapshot) {
        ...
        if (snapshot.config.sessionId != config_.sessionId ||
            ...
            !RtsScriptSessionArchive::decode(
                snapshot.sessionArchive, session_, scripts_, error) ||
            session_.simulation().nextExpectedTick() != snapshot.nextTick) {
            lastScriptError_ = std::move(error);
            return false;
        }
        std::uint64_t restoredHash = 0;
        if (!RtsScriptSessionArchive::authoritativeHash(
                session_, scripts_, restoredHash, error) ||
            restoredHash != snapshot.authoritativeHash) {
            lastScriptError_ = std::move(error);
            return false;
        }
...
private:
    ...
    bool validateFrame(
        ...
        for (const auto& command : frame.commands) {
            if (command.targetTick != frame.tick ||
                command.issuer != owner.issuer || command.sequence == 0 ||
    ...
    bool runScripts(std::uint64_t tick) {
        const auto result = scripts_.processCompletedTick(tick);
        if (result == RtsScriptTickResult::Processed ||
            result == RtsScriptTickResult::NoCallbacks) {
            return true;
        }
        if (!scripts_.errors().empty()) {
            lastScriptError_ = scripts_.errors().back().error;
        } else {
            lastScriptError_.code =
                realscript::runtime::ErrorCode::ExecutionTerminated;
            lastScriptError_.message = "RTS script session step failed";
        }
        return false;
    }

    bool stateHash(std::uint64_t& output) const {
        realscript::runtime::RuntimeError error;
        return RtsScriptSessionArchive::authoritativeHash(
            session_, scripts_, output, error);
    }

    bool recordCompletedTick(std::uint64_t tick) {
        std::uint64_t hash = 0;
        if (!stateHash(hash)) return false;
    ...
    bool captureCurrent(bool force) {
        ...
        checkpoint.resumeTick = session_.simulation().nextExpectedTick();
        realscript::runtime::RuntimeError error;
        if (!RtsScriptSessionArchive::authoritativeHash(
                session_, scripts_, checkpoint.authoritativeHash, error)) {
            lastScriptError_ = std::move(error);
            return false;
        }
        checkpoint.archive = RtsScriptSessionArchive::encode(
            session_, scripts_, error);
    ...
    bool restoreCheckpoint(const sim::RollbackCheckpoint& checkpoint) {
        realscript::runtime::RuntimeError error;
        if (!RtsScriptSessionArchive::decode(
                checkpoint.archive, session_, scripts_, error) ||
            session_.simulation().nextExpectedTick() != checkpoint.resumeTick) {
            lastScriptError_ = std::move(error);
            return false;
        }
        std::uint64_t hash = 0;
        if (!RtsScriptSessionArchive::authoritativeHash(
                session_, scripts_, hash, error) ||
            hash != checkpoint.authoritativeHash) {
            lastScriptError_ = std::move(error);
            return false;
        }
        return true;
    ...
    bool replayRange(
        ...
        for (std::uint64_t tick = beginTick; tick < endExclusive; ++tick) {
    ...
    RtsLockstepAdvanceResult rollbackAndReplay() {
        ...
        if (!restoreCheckpoint(checkpoint) ||
            !coordinator_.rewindSimulatedThrough(checkpoint.resumeTick)) {
    ...
    realscript::runtime::RuntimeError lastScriptError_;
    bool started_{};
};
