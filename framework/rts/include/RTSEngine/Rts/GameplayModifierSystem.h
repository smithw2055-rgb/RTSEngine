...
public:
    ...
    std::uint32_t constructionTicks(
        std::uint32_t teamId,
    ...
    std::int32_t bounty(
        ...
        return std::max<std::int32_t>(
            0,
            ScaleGameplayValue(
                baseValue, profile(teamId).bountyMultiplier));
    }
    ...
    void applyEntity(ecs::World& world, ecs::Entity entity) const {
        ...
        if (auto* armor = world.try_get<Armor>(entity)) {
            const auto bonus = base->building
                ? 0
                : modifiers.unitArmorAdd;
    ...
    void retimeTeam(ecs::World& world, std::uint32_t teamId) const {
        ...
        for (const auto entity : world.view<Team, ResearchQueue>()) {
            const auto* team = world.try_get<Team>(entity);
            auto* queue = world.try_get<ResearchQueue>(entity);
            if (!team || team->id != teamId || !queue) continue;
            for (auto& item : queue->items) {
                const auto base = item.baseRequiredTicks == 0
                    ? item.requiredTicks
